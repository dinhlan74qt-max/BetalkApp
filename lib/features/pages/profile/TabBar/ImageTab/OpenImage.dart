...
import '../../../../../data/models/PostModel/DetailsComment.dart';
import '../../../../../data/models/userModel.dart';
import '../../../../../data/server/ServerConfig.dart';
import '../../../../../data/server/postApi/PostApi.dart';
...
class _OpenImageState extends State<OpenImage> {
  ...
  bool _isPlay = false;
  bool _isLoadingInformation = true;
  ...
  void _initializeCurrentPage() {
    ...
    if (_currentPage < widget.articleModel.mediaItems.length - 1) {
      _initializeVideoAtIndex(_currentPage + 1);
    }
  }
  void _initializeVideoAtIndex(int index) {
    if (index < 0 || index >= widget.articleModel.mediaItems.length) return;
    if (widget.articleModel.mediaItems[index].type != 'video') return;
    if (_videoControllers.containsKey(index)) return;

    final controller = VideoPlayerController.networkUrl(
      Uri.parse(widget.articleModel.mediaItems[index].url),
    );
  ...
  void _precacheImageAtIndex(int index) {
    if (!mounted) return;
    if (index < 0 || index >= widget.articleModel.mediaItems.length) return;
    if (widget.articleModel.mediaItems[index].type != 'image') return;

    final imageProvider = NetworkImage(widget.articleModel.mediaItems[index].url);
  ...
  Future<void> getMusicById() async{
    if(widget.articleModel.idMusic == '0') return;
    final String response = await rootBundle.loadString(
      'assets/data/songs.json',
    );
    final List songs = jsonDecode(response);
    for(var song in songs){
      if(song['id'] == widget.articleModel.idMusic){
        player.setUrl(song['file']);
        setState(() {
          music = {
            "id": song['id'],
            "name": song['name'],
  ...
  Future<void> getComment() async{
    final List<DetailsComment> listGetApi = await PostApi.getComment(widget.articleModel.articleID);
  ...
  void setUpParameter(){
    likeCount = widget.articleModel.likeCount;
    commentCount = widget.articleModel.commentCount;
    shareCount =  widget.articleModel.sharesCount;
    repostCount =  widget.articleModel.repostCount;
    checkInformation();
  }
  Future<void> checkInformation() async{
    final result = await PostApi.checkInformationPost(widget.articleModel.articleID, widget.userModel.id);
    final bool isLiked = result['isLiked'];
    final bool isShared = result['isShared'];
    final bool isReposted = result['isReposted'];
    print(isLiked);
    print(isShared);
    print(isReposted);
    setState(() {
        ...
        children: [
          Container(
            ...
            child: Container(
              ...
              child: CircleAvatar(
                radius: 16.r,
                backgroundImage: CachedNetworkImageProvider(widget.userModel.avatarUrl),
              ),
            ),
          ),
          SizedBox(width: 10.w),
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (widget.articleModel.idMusic != '0')
                  Row(
                    children: [
      ...
      children: [
        SizedBox(
          ...
          child: PageView.builder(
            ...
            itemCount: widget.articleModel.mediaItems.length,
            itemBuilder: (context, index) {
              final item = widget.articleModel.mediaItems[index];
              return Container(
                color: Colors.black,
        ...
        // Dots indicator
        if (widget.articleModel.mediaItems.length > 1)
          Positioned(
            top: 8.h,
            left: 0,
            right: 0,
            child: Row(
              ...
              children: List.generate(
                widget.articleModel.mediaItems.length,
                    (index) => Container(
          ...
          Positioned(
              ...
              child: GestureDetector(
                ...
                child: Container(
                    ...
                    child: _isPlay ? Icon(Icons.volume_up,color: Colors.white,) : Icon(Icons.volume_off,color: Colors.white,)
                ),
              )
          )
      ],
  ...
  Widget _buildActions(BuildContext context) {
    if(_isLoadingInformation){
      return _buildSkeletonActions();
    }
    return Padding(
      padding: EdgeInsets.symmetric(horizontal: 12.w, vertical: 8.h),
      child: Row(
        children: [
          GestureDetector(
            onTap: () {
              PostApi.likeAndeDislike(widget.articleModel.articleID, widget.userModel.id);
              if(_isLiked){
                setState(() {
                  _isLiked = !_isLiked;
                  likeCount--;
          ...
          GestureDetector(
              onTap: () async{
                await _handleShare(widget.articleModel.articleID, widget.userModel.id);
              },
              child: FaIcon(FontAwesomeIcons.paperPlane,color:_isShared ? Colors.blue : Colors.black,)
          ),
          SizedBox(width: 2.w,),
          Text(
          ...
          GestureDetector(
            onTap: () async{
              await _handleRepost(widget.articleModel.articleID, widget.userModel.id);
            },
            child:FaIcon(FontAwesomeIcons.repeat,size: 20.sp,color: _isSaved ? Colors.yellow : Colors.black,),
          ),
        ],
      ),
    );
  ...
  Widget _buildSkeletonActions() {
    return Shimmer.fromColors(
      ...
      child: Padding(
        ...
        child: Row(
          children: [
            _skeletonBox(width: 26.w, height: 26.h),
            SizedBox(width: 6.w),
            _skeletonBox(width: 20.w, height: 12.h),
            SizedBox(width: 16.w),

            _skeletonBox(width: 22.w, height: 22.h),
            SizedBox(width: 6.w),
            _skeletonBox(width: 20.w, height: 12.h),
            SizedBox(width: 16.w),
  ...
  Widget _buildLikesAndCaption() {
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          if (widget.articleModel.content != '0') ...[
            SizedBox(height: 4.h),
            RichText(
              text: TextSpan(
                style: TextStyle(
                ...
                children: [
                  ...
                  TextSpan(text: widget.articleModel.content),
                ],
              ),
            ),
          ],
            ...
            builder: (BuildContext context, StateSetter setModalState) {
              return Padding(
                padding: EdgeInsets.only(
                  bottom: MediaQuery.of(context).viewInsets.bottom, // ✅ Đẩy lên khi bàn phím hiện
                ),
                          ...
                          Expanded(
                            child: GestureDetector(
                              ...
                              child: ListView.builder(
                                ...
                                keyboardDismissBehavior: ScrollViewKeyboardDismissBehavior.onDrag, // ✅ Đóng bàn phím khi scroll
                                ...
                                children: [
                                  ...
                                  Expanded(
                                    child: TextField(
                                      ...
                                      textInputAction: TextInputAction.send, // ✅ Nút send trên bàn phím
  ...
  Future<void> _handleSendComment(BuildContext context, StateSetter setModalState,) async {
  ...
  Future<void>_handleShare(String postId, String userId) async{
    ...
    if(_isShared){
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Đã hủy share")),
      );
      setState(() {
        shareCount--;
        _isShared = false;
  ...
  Future<void> _handleRepost(String postId, String userId) async{
    ...
    final result = await PostApi.repost(widget.articleModel.articleID, widget.userModel.id);
    if(!result){
      setState(() {
        repostCount--;
        _isSaved = false;
